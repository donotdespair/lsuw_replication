...
Rcpp::sourceCpp("src_cpp/bsvar_sv.cpp")
...
starting_values$sigma2_omega = rep(10, 2)
VB              = spec$identification$get_identification()

burn          <- bsvar_sv_cpp(S = S_burn, Y = t(yy), prior, VB, starting_values, thin = 1, centred_sv = TRUE, show_progress = TRUE)
post          <- bsvar_sv_cpp(S = S, Y = t(yy), prior, VB, starting_values = burn$last_draw, thin = 1, centred_sv = TRUE, show_progress = TRUE)
...
# save(post, file = paste0("results/sv_post_", T, "_", id, ".rda"))
...
save(Bm, sm, Sm, file = paste0("results/ce10_mse_", T, "_", id, ".rda"))
