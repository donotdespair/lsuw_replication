/*
 * In this file the SVAR(0)-SV model estimation and SDDR computation
 * are adjusted for Chan (2018, ER) paper prior for omega_n ~ N(0,10)
 */
...
Rcpp::List bsvar_sv_chan_cpp (
  ...
  for (int s=0; s<S; s++) {
    ...
    for (int n=0; n<N; n++) {
      ...
      List sv_n = svar_nc1( h_tmp, rho_tmp, omega_tmp, sigma2v_tmp, s2o_tmp, S_tmp, U_tmp, prior );
...
// [[Rcpp::export]]
arma::vec verify_volatility_sv_chan_cpp (
  ...
  const double  sigma2_omega    = posterior_sigma2_omega(0, 0);
  ...
  double  log_denominator = - 0.5 * log(2 * M_PI);
  log_denominator        -= 0.5 * log(sigma2_omega);
  ...
  for (int s = 0; s < S; s++) {
    ...
    mat     residuals       = log(square(posterior_B.slice(s) * Y ));
      ...
      double  V_omega         = pow(as_scalar(posterior_h.slice(s).row(n) 
                                                * diagmat(sigma_S_inv) 
                                                * trans(posterior_h.slice(s).row(n))) 
                                                + pow(sigma2_omega, -1), -1);
      double  omega_bar       = V_omega * as_scalar(posterior_h.slice(s).row(n) 
                                                      * diagmat(sigma_S_inv) 
                                                      * trans(residuals.row(n) - alpha_S));
...
} // END verify_volatility_sv_chan_cpp
...
# starting_values$h = matrix(rnorm(N * T, sd = 0.01), N, T)
# starting_values$S = matrix(1, N, T)
...
# save(sddr, file = paste0("bsvar_sv_chan_",iteration,".rda"))
