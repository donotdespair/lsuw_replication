...
/*______________________two normalisation functions______________________*/  
...
void sample_B_heterosk1_nxi (
    ...
    const mat   B_hat,
    const int   xi_max_iterations = 5000
) {
  ...
  bool  is_xi_positive      = true;
  int   is_xi_it            = 0;
  
  while ( is_xi_positive && is_xi_it < xi_max_iterations ) {
    for (int n=0; n<N; n++) {
    ...
    mat cov_inv               = covariance_inv_bsvars(B_hat, square(aux_sigma), T, communication);
    mat P                     = pick_permutation_bsvars(aux_B_tmp, B_hat, T, cov_inv, allP, allS, communication);
    mat aux_B_tmp_norm        = P * aux_B_tmp;
    
    double xi                 = - aux_B_tmp_norm(2,0) / aux_B_tmp_norm(2,2);
    
    is_xi_positive            = xi >= 0;
    is_xi_it++;
  
  } // END while (is_xi_positive)  
  
  Rcout << " xi iterations: " << is_xi_it << endl;
  
  if ( is_xi_it < xi_max_iterations ) {
    aux_B                     = aux_B_tmp;
  }
...
Rcpp::List bsvar_sv_nxi_cpp (
  ...
  for (int ss=0; ss<SS; ss++) {
    ...
    // sample aux_B
    sample_B_heterosk1_nxi(aux_B, aux_A, aux_hyper, aux_sigma, Y, X, prior, VB, B_hat);
