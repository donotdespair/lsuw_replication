...
# save(post, spec, sddr, file = "spartan/results/tax06n.rda")

# # This is worse!
# qq_normL_MR       = normalise_jaro_bsvar_sv(post$posterior, B0hat_MR)
# post              = specify_posterior_bsvar_sv$new(post$last_draw, qq_normL_MR)
# sddr_MR           = verify_volatility(post)
# sf_MR             = TranstoMR(post$posterior)


# recompute all outcomes
########################################################
sddr$logSDDR
sddr$log_SDDR_se
...
Bhat_index        = which.min(apply(qq_normL_PM$B, 3, function(x){sum((x-B0hat_BP)^2)}))
B0hat_PM          = post$posterior$B[,,Bhat_index]
