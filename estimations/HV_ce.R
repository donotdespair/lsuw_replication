...
# HV_ce: heteroskedasticity verification plots
# for the centred parameterisation as requested by R2
############################################
...
load("results/tax23_ce.rda")
post23 = normalise_jaro_bsvar_sv(post$posterior, B0hat_BP)
class(post23) = "PosteriorBSVARSV"

load("results/tax06_ce.rda")
post06 = normalise_jaro_bsvar_sv(post$posterior, B0hat_BP)
class(post06) = "PosteriorBSVARSV"

load("results/taxMR_ce.rda")
postMR = normalise_jaro_bsvar_sv(post$posterior, B0hat_BP)
class(postMR) = "PosteriorBSVARSV"

data("us_fiscal_lsuw")
us_fiscal_lsuw = ts(
      us_fiscal_lsuw[zoo::index(us_fiscal_lsuw) < 2023.75,], 
      start = c(1948, 1), 
      frequency = 4
)
...
# post23 |> compute_conditional_sd() |> plot()
# post06 |> compute_conditional_sd() |> plot()
# postMR |> compute_conditional_sd() |> plot()
...
# system("cp HV_ce.pdf /Users/twozniak/Research/Carnaval-gulasz-Fei/fei-tomasz-sv-prior/cg_paper/01 JoE R&R1/graph_HV_ce.pdf")
